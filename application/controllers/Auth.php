...
class Auth extends CI_Controller {
	...
	$dat['trans'] =	$this->userm->fetchWithdrawTransaction();
	$this->load->view("auth/transactions",$dat);}
	else{
		redirect("auth/");
	}
	}
		...
		if($this->ion_auth->is_admin()){
	$dat['trans'] =	$this->userm->fetchPendingTransaction();
	$this->load->view("auth/transactions",$dat);}
	else{
		redirect("auth/");
	}
	...
	public function waitingTransactions(){
		if($this->ion_auth->is_admin()){
	$dat['trans'] =	$this->userm->fetchWaitingTransaction();
	$this->load->view("auth/transactions",$dat);}
	else{
		redirect("auth/");
	}
	...
	public function completeTransaction($id){
		$this->userm->completeTrans($id);
		redirect("auth/backworld");
	}
	...
	public function refferal($id){
		$res = $this->userm->checkrefferal($id);
		echo $res;
	}

	public function transDetails($id,$type){
			$dat['detail'] = $this->userm->fetchTransDetail($id,$type);
	...
	public function checkCredentional(){
...
// profile image manipulation
		$filename = '';
			...
			$tmp = $_FILES['company']['tmp_name'];
			$filename = $_FILES['company']['name'];
			$maxsize    = 11097152;
			$ext = pathinfo($filename,PATHINFO_EXTENSION);
			if($_FILES['company']['size'] >= $maxsize)
				$errors[]  = "File size is too large File must be less than 1 mb";
			if(!in_array($ext,$allowed))
			$errors[] = "Invalid file type. Only  JPG, GIF ,JPEG and PNG types are accepted.";
			if(count($errors)==0){
			$target = "assets/images/".$identity.".$ext";
	...
	$filename =	$user->company;
	if(isset($_FILES['company']['name']))
    if (is_uploaded_file($_FILES['company']['tmp_name'])) {
        $errors     = array();
        $allowed =  array('gif','png' ,'jpg','jpeg');
        $tmp = $_FILES['company']['tmp_name'];
        ...
        $ext = pathinfo($filename,PATHINFO_EXTENSION);
        if($_FILES['company']['size'] >= $maxsize)
            $errors[]  = "File size is too large File must be less than 2 mb";
        if(!in_array($ext,$allowed))
           $errors[] = "Invalid file type. Only  JPG, GIF and PNG types are accepted.";
        if(count($errors)==0){
        $target = "assets/images/".$user->username.".$ext";
		...
		$img = base_url()."assets/images/".$user->company;
